...
class VESCBridgeNode(Node):
    def __init__(self):
        ...
        self.kickstart_duration = 0.3  # seconds
        self.kickstart_duty = 0.1      # 10% duty

        # Subscribe to RPM command topic
        ...
        # Timer to send control commands at 50 Hz
        self.create_timer(0.05, self.control_loop)

        # Timer to publish voltage at 1 Hz
        self.create_timer(1, self.publish_battery_state)

        self.get_logger().info(f"🛠️ Kickstart mode: {'enabled' if self.use_kickstart else 'disabled'}")

    def initialize_vesc(self, port, max_retries=3):
        for attempt in range(1, max_retries + 1):
            try:
                self.get_logger().info(f"⏳ Connecting to VESC (Attempt {attempt})...")
                motor = VESC(serial_port=port)
                self.get_logger().info("✅ VESC connected.")
                return motor
            except (ValueError, TypeError) as e:
                self.get_logger().warn(f"⚠️ VESC init failed: {e}")
                time.sleep(1)
            except Exception as e:
                self.get_logger().error(f"❌ Unexpected VESC error: {e}")
                break
        self.get_logger().error("🚨 VESC connection failed after retries.")
        return None
        ...
        try:
            if self.use_kickstart:
                if self.last_rpm_cmd == 0 and abs(self.latest_rpm_cmd) > 100 and not self.kickstart_active:
                    ...
                    self.get_logger().info(f"⚡ Kickstarting with duty: {duty:.2f}")
                    ...
                    if elapsed >= self.kickstart_duration:
                        ...
                        self.get_logger().info(f"🔁 Switching to RPM: {self.latest_rpm_cmd}")
                    else:
                        duty = self.kickstart_duty if self.latest_rpm_cmd > 0 else -self.kickstart_duty
                        self.vesc.set_duty_cycle(duty)
                else:
            ...
            self.get_logger().warn(f"⚠️ VESC command error: {e}")
    ...
    def publish_battery_state(self):
        ...
        try:
            ...
            self.get_logger().warn(f"⚠️ Failed to read VESC voltage: {e}")
